...
function collectLKGrades(passedLKGrades){
    return [
...
function calculateTotalPoints(passedGKGrades, passedLKGrades, passedAbiturGrades){
    ...
    if ((deficitsGK + deficitsLK) > 5) {
        return "Du hast in den einzubringenden Kursen insgesamt mehr als <span class='red-letter'>fünf Defizite</span>. Damit wärst du nicht zur Abiturprüfung zugelassen.";
    } else if (deficitsLK > 3) {
        return "Du hast im Leistungskursbereich mehr als <span class='red-letter'>drei Defizite</span>. Damit wärst du nicht zur Abiturprüfung zugelassen.";
    } else {
        ...
        if ((pointsBlockI) < 200) {
            return "Du hast in den einzubringenden Kursen weniger als <span class='red-letter'>200 Punkte</span> erreicht. Damit wärst du nicht zur Abiturprüfung zugelassen.";
        } else {
            const pointsBlockII = 5 * passedAbiturGrades.reduce((acc, curr) => acc + curr, 0);
            if (pointsBlockII < 100) {
                return "Du hast in den Abiturprüfungen mit <span class='red-letter'>" + pointsBlockII + " Punkten</span> weniger als <span class='red-letter'>100 Punkte</span> erreicht. Damit hättest du die Abiturprüfung nicht bestanden. Mündliche Nachprüfungen sind ggf. möglich."
            } else if (checkAbiGKDeficits(passedAbiturGrades) >= 2) {
                return "Du hast in den Grundkursen im Abiturbereich <span class='red-letter'>" + checkAbiGKDeficits(passedAbiturGrades) + " Defizite</span>. Damit hättest du die Abiturprüfung nicht bestanden. Mündliche Nachprüfungen sind ggf. möglich."
            } else if (checkAbiLKDeficits(passedAbiturGrades) >= 2) {
                return "Du hast in den Leistungskursen im Abiturbereich <span class='red-letter'>" + checkAbiLKDeficits(passedAbiturGrades) + " Defizite</span>. Damit hättest du die Abiturprüfung nicht bestanden. Mündliche Nachprüfungen sind ggf. möglich."
            } else {
                ...
                return "Du hast die Abiturprüfung mit einem <span class='red-letter'>Schnitt</span> von <span class='red-letter'>" + avgAbitur.toFixed(1) + "</span> bestanden.";
            }
        }
    }
}
...
function resetForm() {
    ...
    const resultArea = document.getElementById('resultArea');
    resultArea.innerHTML = "<i>Hier werden später die <span class='red-letter'>Ergebnisse<span> angezeigt.</i>";
}
...
function validateInputs() {
    ...
    inputs.forEach(input => {
        if (input.style.display !== 'none' && (input.value === '' || input.value < 0 || input.value > 15)) {
            allValid = false;
            errorMessage = 'Bitte gib für alle sichtbaren Felder Noten zwischen <span class="red-letter">0</span> und <span class="red-letter">15</span> ein.';
        }
    });
    ...
    if (!group1Checked) {
        allValid = false;
        errorMessage += ' Du hast kein drittes bzw. viertes Abiturfach ausgewählt.';
    }
    ...
    if (!allValid) {
        ...
        resultArea.innerHTML = 'Alle Eingaben sind korrekt!';
        return true;
    }
}
